...
let biggerThanLuke = (arr) => {
  // This one was really tough, GPT helped out here.
  // Solution code here...
  if (arr.length === 0) {
    return '';
  }
  // Find Luke Skywalker's mass
  const luke = arr.find(character => character.name === 'Luke Skywalker');
  // If Luke is not in the array, return an empty string
  if (!luke) return '';

  const lukeMass = parseInt(luke.mass, 10);

  const biggerCharacters = arr.filter(character => parseInt(character.mass, 10) > lukeMass);

  if (biggerCharacters.length === 0) return '';
...
const detectTicTacToeWin = (board) => {
  // This one was really tough, GPT helped out a bunch here.
  const checkLine = (row1, col1, row2, col2, row3, col3) => {
